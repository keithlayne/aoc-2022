...
with grid (row, col, height) as (
  ...
  select max(
    coalesce((select i from generate_subscripts(u, 1) as s (i) where u[i] >= height limit 1), cardinality(u)) *
    coalesce((select i from generate_subscripts(l, 1) as s (i) where l[i] >= height limit 1), cardinality(l)) *
    coalesce((select i from generate_subscripts(r, 1) as s (i) where r[i] >= height limit 1), cardinality(r)) *
    coalesce((select i from generate_subscripts(d, 1) as s (i) where d[i] >= height limit 1), cardinality(d))
  )
  from views
